...
# Wrapper over the Label Powerset multi-label classifier from skmultilearn with a MultinomialNB base classifier from sklearn
class LMWrapper(Model):
    ...
    def build_representation( self, x, fit=False ):
        ...
        auxX1 = [ ' '.join( [ 'w_'+str(token) for token in instance[0:2000,] if token != 0 ] ) for instance in x ]
        auxX1 = [ 'main \n ' + x for x in auxX1 ]
        if fit : auxX1 = self.vect1.fit_transform(auxX1)
        else : auxX1 = self.vect1.transform(auxX1)
        auxX2 = [ ' '.join( [ 'w_'+str(token) for token in instance[2000:3600,] if token != 0 ] ) for instance in x ]
        auxX2 = [ 'aux \n ' + x for x in auxX2 ]
